...
- **Order id format.** The `order_id` is the first 24 characters of a gateway-namespaced SHA-256 digest of the effective idempotency key — alphanumeric and within FastPay's 8–32 character limit. It stays stable across charge retries, so a retried charge never double-creates a payment.
